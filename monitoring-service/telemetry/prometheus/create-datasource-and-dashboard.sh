...
if ! curl --retry 5 --retry-connrefused --retry-delay 0 -sf http://grafana:9094/api/dashboards/name/prom; then
    curl -sf -X POST -H "Content-Type: application/json" \
         --data-binary '{"name":"prom","type":"prometheus","url":"http://prometheus:9090","access":"proxy","isDefault":true}' \
         http://grafana:9094/api/datasources
fi
...
curl --retry-connrefused --retry 5 --retry-delay 0 -sf \
     ...
     http://grafana:9094/api/dashboards/import 